...
class PreviewDetails {
  ...
  PreviewDetails(this.width, this.height, this.sensorOrientation, this.textureId,this.minZoom, this.maxZoom);
}
...
class QrMobileVision {
  ...
  static Future<PreviewDetails> start({
    ...
    num? minZoom;
    num? maxZoom;
    if(details["minzoom"]!=null){
      minZoom=details["minzoom"];
      maxZoom=details["maxzoom"];
    }

    return PreviewDetails(surfaceWidth, surfaceHeight, orientation, textureId,minZoom,maxZoom);
  }


  static Future<double> getMaxZoom() async{
    var data=await _channel.invokeMethod("getMaxZoom").catchError(print);
    assert(data is Map<dynamic, dynamic>);
    return data["maxzoom"] as double;
  }

  static Future<double> getMinZoom() async{
    var data=await _channel.invokeMethod("getMinZoom").catchError(print);
    assert(data is Map<dynamic, dynamic>);
    return data["minzoom"] as double;
  }


  static Future<double> getCurZoom() async{
    var data=await _channel.invokeMethod("getCurZoom").catchError(print);
    assert(data is Map<dynamic, dynamic>);
    return data["zoom"] as double;
  }


  static Future setZoom(double zoomlevel) async{
    var data=await _channel.invokeMethod("setZoom",{"zoomlevel":zoomlevel}).catchError(print);
   // assert(data is Map<dynamic, dynamic>);
    return ;
  }
